...
import {interfacelist,addinterface,delInterFace,createInterface,getListPro,addproject,delproject} from '../../api/interface'
const tools = {
  state: {
    token: getToken(),
    interfaceList: [],
  ...
  actions: {
    ...
    GetAllInterface({commit, state},params){
      return new Promise((resolve, reject) => {
        ...
        interfacelist(state.token,params).then((response) => {
    ...
    //查询多有项目
    GetAllProject({commit,state},params) {
      return new Promise((resolve,reject)=>{
        getListPro(getToken(),params).then((response)=>{
          var data = response.body

            commit('SET_PROLIST',data)
    ...
    AddProject({commit,state},params) {
      return new  Promise((resolve,reject)=>{
        addproject(state.token,params.proname,params.prodes,params.type).then((response)=>{
          resolve(response)
        }).catch((error)=>{
          reject(error)
        })
    ...
    //删除一个项目
    DelPro({commit,state},params){
      return new Promise((resolve,reject)=>{
        delproject(state.token,params.name).then((response)=>{
           resolve(response)
        }).catch((error)=>{
           reject(error)
        })
