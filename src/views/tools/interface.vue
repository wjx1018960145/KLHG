...
<el-col :span="24"style="padding-bottom: 0px;width: 70%">
  <el-table
    :data="tableData5"
    style=" float: left" row-key="name" :expand-row-keys="expands" @row-click="showdetail">
            ...
            <el-form :inline="true" ref="Addform" :model="Addform" :rules="rules" label-width="140px">
              ...
              <el-form-item label="所属工程" prop="project">
                ...
                <span class="el-dropdown-link">
                  {{this.tmpdropdown}}<i class="el-icon-arrow-down el-icon--right"></i>
                </span>
                  <el-dropdown-menu slot="dropdown">
                    <el-dropdown-item v-for="(item, index) in proList" :command="item.id">{{item.name}}</el-dropdown-item>
              ...
              <!--<el-input name="author" v-model="Addform.author" value="" :disabled="true"  required :maxlength="100">-->
              <!--创建者-->
              <!--</el-input>-->
              <!--</el-form-item>-->
              <el-form-item label="接口描述">
...
<script>
  ...
  export default {
    components: {ElCol, Sticky, jsonEditor, MDinput},
    data(){
    ...
    methods: {
      handleCommand(command) {
        for (var  index in this.proList){
            if (this.proList[index].id == command){
                this.tmpdropdown = this.proList[index].name
            }
        }
        this.$message('click on item ' + command);
      },
      initData(index){
        this.$store.dispatch('GetAllInterface', index).then(() => {
      ...
      AddIF(){
        ...
        Request.requestAllProToken(params).then((res) => {
            var data = res.data
            if(data.code == 20000){
                _self.proList = [];
              _self.proList = data.body
            }else {
